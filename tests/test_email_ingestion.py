...
def test_gmail_multipart_message_extracts_multiple_vacancies_and_filters_urls():
    ...
    assert {(item["source"], item["external_id"]) for item in candidates} == {
        ("linkedin", "111"),
        ("headhunter", "123"),
    }
...
def test_vacancy_url_accepts_only_known_patterns_and_canonicalizes_tracking():
    assert vacancy_url("https://linkedin.com/jobs/view/111?trk=x") == (
        "linkedin",
        "111",
        "https://www.linkedin.com/jobs/view/111",
    )
    assert vacancy_url("https://hh.ru/vacancy/123?utm_source=x") == ("headhunter", "123", "https://hh.ru/vacancy/123")
    assert vacancy_url("https://headhunter.ge/vacancy/123?from=email") == (
        "headhunter",
        "123",
        "https://headhunter.ge/vacancy/123",
    )
...
def test_email_sync_deduplicates_messages_urls_and_is_idempotent(monkeypatch, tmp_path):
    ...
    assert read_json(preferences.outputs.output_dir() / preferences.outputs.email_state_file)[
        "processed_message_ids"
    ] == ["<alert-1@example.test>", "<alert-duplicate@example.test>"]
...
def test_email_sync_uses_body_peek_and_does_not_change_flags(monkeypatch, tmp_path):
    ...
    sync_email_alerts(
        ...
        hh_lookup=lambda vacancy_id: (HEADHUNTER_RESPONSE["items"][0], BatchStats()),
    )

    commands = FakeIMAP.instances[0].commands
    assert ("select", "JobAlerts", True) in commands
    assert any(
        command[:3] == ("uid", "FETCH", "1") and "BODY.PEEK[]" in " ".join(str(part) for part in command)
...
def test_dry_run_does_not_write_email_outputs(monkeypatch, tmp_path):
    ...
    result = sync_email_alerts(
        preferences,
        since_days=30,
        dry_run=True,
        imap_factory=lambda host, port: FakeIMAP(messages),
        hh_lookup=lambda vacancy_id: (None, BatchStats()),
    )
...
def test_email_hh_candidates_flow_through_existing_pipeline(monkeypatch, tmp_path):
    ...
    queue = (preferences.outputs.output_dir() / preferences.outputs.linkedin_email_queue_file).read_text(
        encoding="utf-8"
    )
