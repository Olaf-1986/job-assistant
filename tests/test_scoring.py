...
from tests.fixtures.himalayas_records import BUSINESS_ANALYST, CRYPTO_ROLE, INTERNSHIP, JIRA_USER, JUNIOR_ROLE, STRONG_JIRA_ADMIN
...
def test_jira_admin_requires_atlassian_platform_context():
    raw = {**BUSINESS_ANALYST, "description": "<p>Define roles, workflows and automation for marketing operations.</p>"}
    vacancy = scored(raw)
...
def test_international_english_signal_does_not_add_legacy_bonus():
    plain = scored({**BUSINESS_ANALYST, "excerpt": "", "description": "<p>Requirements gathering and BPMN.</p>"})
    assert not any("English/international" in line for line in plain.score_breakdown)
    international = scored({**BUSINESS_ANALYST, "description": "<p>International distributed team. Requirements gathering and BPMN.</p>"})
