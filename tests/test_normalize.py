...
from tests.fixtures.himalayas_records import BUSINESS_ANALYST, MALFORMED, RUSSIAN_ROLE
...
def test_normalize_warns_on_invalid_salary_without_crashing():
    preferences = load_preferences()
    bad_salary = {**BUSINESS_ANALYST, "guid": "bad-salary", "minSalary": "not-a-number"}
    ...
    assert "invalid numeric value for minSalary" in normalized[0].warnings[0]
